 ...
 */
UCLASS()
class VOXELGAME_API UPerlinNoise : public UObject
{
	GENERATED_BODY()
...
public:
	...
	float SimplePerlin(float x, float y, float z);

	UFUNCTION(BlueprintCallable)
	float OctavePerlin(float x, float y, float z, int32 octaves, float persistence);
...
private:
	...
	TArray<int32> p; 													// Doubled permutation to avoid overflow
